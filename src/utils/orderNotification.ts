...
export function buildOrderNotification(order: Order, purpose: CustomerNotificationPurpose, orderUrl: string): OrderNotificationContent {
  ...
  switch (purpose) {
    ...
    case 'turnitin-report':
      return {
        subject: `Your Turnitin report is ready — ${order.orderId}`,
        message: `${greeting}\n\nYour Turnitin report is ready. Use your secure order link to view and download the labelled report files.\n${reference}\n\n${orderUrl}`
      };
    case 'complete':
      return {
        subject: `Your Hack-Key Tech order is complete — ${order.orderId}`,
        message: `${greeting}\n\nYour order is complete. Use your secure order link to view your deliverables and any remaining instructions.\n${reference}\n\n${orderUrl}`
      };
    default:
      return {
        subject: `Update on your Hack-Key Tech order ${order.orderId}`,
        message: `${greeting}\n\nThere is an update on your order. Open your secure order link to see its current status and next step.\n${reference}\n\n${orderUrl}`
      };
  }
}
