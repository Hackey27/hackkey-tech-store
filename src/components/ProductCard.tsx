  ...
  return (
    ...
    >
      ...
      <div className={`px-5 sm:px-6 py-4 border-t flex flex-wrap items-center justify-between gap-3 ${product.kind === 'product' ? 'border-[#025656] bg-[#014040]' : 'border-[#e2ecea] bg-[#f8fbfa]'}`}>
        ...
        {payable ? <div className="ml-auto flex flex-wrap justify-end gap-2"><button id={`product-buy-btn-${product.itemId}`} onClick={(event) => { event.stopPropagation(); onBuyNowClick(product); }} className={`flex items-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-black ${product.kind === 'product' ? 'border border-[#05ef28] bg-[#05ef28] text-[#014040] hover:bg-[#20f43d]' : 'bg-[#05ef28] text-[#014040] hover:bg-[#04d824]'}`}><CreditCard className="h-3.5 w-3.5" />Buy now</button><button id={`product-cart-btn-${product.itemId}`} onClick={(event) => { event.stopPropagation(); onAddToCart?.(product); }} className={`flex items-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-black ${product.kind === 'product' ? 'border border-white bg-white text-[#014040] hover:bg-[#edf5f3]' : 'bg-[#014040] text-white hover:bg-[#025656]'}`}><ShoppingCart className="h-3.5 w-3.5" />Add to cart</button></div> : <button onClick={(event) => { event.stopPropagation(); if (product.kind === 'laptop' && onInterestClick) onInterestClick(product); else onSelect(product); }} className="ml-auto flex items-center gap-1.5 rounded-xl bg-[#014040] px-4 py-2.5 text-xs font-black text-white"><MessageSquareQuote className="h-3.5 w-3.5" />{product.kind === 'laptop' ? 'I am interested' : 'Get a quote'}</button>}
      </div>
    </article>
  );
};
