...
export const CategoryPage: React.FC<CategoryPageProps> = ({
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
      ...
      <header className="rounded-3xl bg-gradient-to-r from-[#014040] to-[#025656] p-6 text-white shadow-sm sm:p-9">
        <p className="text-xs font-black uppercase tracking-[0.16em] text-[#05ef28]">{STORE_COPY.catalog.browseByCategory}</p>
        <h1 className="mt-2 max-w-4xl text-2xl font-black tracking-tight sm:text-4xl">{category.name}</h1>
        {category.tagline && <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-200 sm:text-base">{category.tagline}</p>}
        <p className="mt-4 text-xs font-bold text-[#d9ffe0]">{STORE_COPY.catalog.categoryItems(items.length)}</p>
      </header>
