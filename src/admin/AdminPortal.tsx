        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[#002b2b]/60 p-3 sm:p-8">
          <section className="mx-auto max-w-4xl rounded-3xl bg-white p-5 shadow-2xl sm:p-8">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {selected.fulfilmentStatus === 'awaiting-licence' && <div className="rounded-2xl border p-4"><h4 className="font-black">{ADMIN_COPY.orders.assignLicence}</h4><input className={`${inputClass} mt-3`} placeholder={ADMIN_COPY.orders.manualKey} value={manualKey} onChange={(e) => setManualKey(e.target.value)} /><div className="mt-3 flex flex-wrap gap-2"><button className={primaryButton} disabled={!!busy} onClick={() => act('assign', `/orders/${selected.orderId}/assign-licence`, manualKey ? { manualKey } : {})}><KeyRound className="h-4 w-4" />{manualKey ? ADMIN_COPY.orders.assignLicence : ADMIN_COPY.orders.usePool}</button></div></div>}
