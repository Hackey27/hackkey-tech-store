...
function OrdersSection({ data, user, reload }: { data: AdminData; user: User; reload: () => Promise<void> }) {
  ...
  const [quickLicenceKind, setQuickLicenceKind] = useState<'sales-id' | 'licence'>('licence');
  ...
  const [offlineReason, setOfflineReason] = useState('Paid via MoMo');
  const [offlineError, setOfflineError] = useState('');
  const [editKind, setEditKind] = useState<'sales-id' | 'licence' | 'momo-reference' | null>(null);
  ...
  const assignQuickLicence = async (event: React.MouseEvent, order: Order, licenceId?: string) => {
    ...
    const salesOrder = Boolean(order.customerInputType) || order.deliveryCodeType === 'sales-code' || data.products.some((product) => product.variants.some((variant) => variant.variantId === order.variantId && variant.deliveryCodeType === 'sales-code'));
    if (quickLicenceKind === 'licence' && salesOrder && !manualKey) { setQuickLicenceMessage('Paste the generated licence code.'); return; }
    if (!manualKey && !licenceId) { setQuickLicenceMessage(`Paste a licence or choose the next available ${quickLicenceKind === 'sales-id' ? 'Sales ID' : 'licence'} in stock.`); return; }
    setBusy(`quick-licence-${order.orderId}`); setQuickLicenceMessage(''); setMessage('');
    try {
      const path = quickLicenceKind === 'sales-id' ? 'assign-sales-code' : salesOrder ? 'generated-licence' : 'assign-licence';
      const payload = quickLicenceKind === 'sales-id' ? { licenceId } : salesOrder ? { code: manualKey } : manualKey ? { manualKey } : { licenceId };
      await adminRequest(user, `/orders/${encodeURIComponent(order.orderId)}/${path}`, { method: 'POST', body: JSON.stringify(payload) });
      setQuickLicenceOrderId(''); setQuickLicenceValue(''); setMessage(`${quickLicenceKind === 'sales-id' ? 'Sales ID' : 'Licence'} added to ${order.orderId}.`);
      await reload();
  ...
  const openMomoPayment = (event: React.MouseEvent, order: Order) => {
    event.stopPropagation();
    setOfflineOrderId(order.orderId);
    setOfflineReference('');
    setOfflineReason('Paid via MoMo');
    setOfflineError('');
  };

  const recordPreviewPayment = async () => {
    if (!offlineOrderId || !offlineReason.trim()) return;
    setBusy('preview-offline'); setOfflineError('');
    try {
      await adminRequest(user, `/orders/${encodeURIComponent(offlineOrderId)}/record-offline-payment`, { method: 'POST', body: JSON.stringify({ reference: offlineReference, reason: offlineReason }) });
      setOfflineOrderId(''); setMessage('Offline payment recorded.'); await reload();
    } catch (error) { setOfflineError(messageOf(error)); }
    finally { setBusy(''); }
  };

  const startGuardedEdit = (kind: 'sales-id' | 'licence' | 'momo-reference') => {
    setEditKind(kind); setEditValue(kind === 'momo-reference' ? selected?.offlinePaymentReference || '' : '');
    setEditReason(''); setEditEmail(''); setEditPassword(''); setEditError('');
  };

  const saveGuardedEdit = async () => {
    if (!selected || !editKind) return;
    if (editEmail.trim().toLowerCase() !== user.email?.toLowerCase()) { setEditError('Enter the email address of this signed-in admin account.'); return; }
    if (!editPassword || !editReason.trim()) { setEditError('Your password and a reason are required.'); return; }
    setBusy('guarded-edit'); setEditError('');
    try {
      await reauthenticateWithCredential(user, EmailAuthProvider.credential(editEmail.trim(), editPassword));
      await user.getIdToken(true);
      await adminRequest(user, `/orders/${encodeURIComponent(selected.orderId)}/assigned-value`, { method: 'PUT', body: JSON.stringify({ kind: editKind, value: editValue, reason: editReason }) });
      setEditKind(null); setEditPassword(''); setMessage('Assigned value corrected.'); await reload();
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        ...
        {rows.map((order) => {
          ...
          const isSalesOrder = Boolean(orderInputType) || order.deliveryCodeType === 'sales-code' || orderVariant?.deliveryCodeType === 'sales-code';
          const hasSavedLicence = Boolean(order.activationCodeOrKey || (!isSalesOrder && order.licenceId));
          const quickLicenceOpen = quickLicenceOrderId === order.orderId;
          const nextAvailableLicence = data.licences
            .filter((licence) => licence.variantId === order.variantId && licence.status === 'available' && (quickLicenceKind === 'sales-id' ? licence.codeType === 'sales-code' : licence.codeType !== 'sales-code'))
          ...
          return <div key={order.orderId} role="button" tabIndex={0} onClick={() => { setSelected(order); setMessage(''); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelected(order); }} className="grid w-full cursor-pointer gap-3 rounded-2xl border border-[#cbdcd9] border-b-4 border-b-[#014040] bg-white p-4 text-left shadow-sm hover:bg-[#f3faf8] md:grid-cols-[140px_1fr_1fr_140px] md:items-center">
            ...
            <div className="flex flex-wrap items-center gap-2 border-t border-[#e4efed] pt-3 md:col-span-4" onClick={(event) => event.stopPropagation()}>
              {orderProduct && isSalesOrder && <button type="button" disabled={order.paymentStatus !== 'paid' || Boolean(order.salesCode)} onClick={() => { setQuickLicenceOrderId(order.orderId); setQuickLicenceKind('sales-id'); setQuickLicenceValue(''); setQuickLicenceMessage(''); }} className={secondaryButton}><KeyRound className="h-4 w-4" />{order.salesCode ? 'Sales ID assigned' : 'Assign Sales ID'}</button>}
              {orderProduct && <button type="button" disabled={order.paymentStatus !== 'paid' || hasSavedLicence} title={order.paymentStatus !== 'paid' ? 'The order must be paid first.' : hasSavedLicence ? 'A licence is already attached.' : 'Add a licence without opening the full order.'} onClick={() => { setQuickLicenceOrderId(order.orderId); setQuickLicenceKind('licence'); setQuickLicenceValue(''); setQuickLicenceMessage(''); }} className={secondaryButton}><KeyRound className="h-4 w-4" />{hasSavedLicence ? 'Licence added' : 'Add license'}</button>}
            ...
            {quickLicenceOpen && <section aria-label={`Add licence for ${order.orderId}`} onClick={(event) => event.stopPropagation()} className="rounded-2xl border border-[#9dbbb5] bg-[#f3faf8] p-4 md:col-span-4">
              <div className="flex items-start justify-between gap-3"><div><h4 className="text-sm font-black text-[#014040]">{quickLicenceKind === 'sales-id' ? 'Assign Sales ID' : 'Add license'}</h4><p className="mt-1 text-xs text-slate-500">{order.productName} · {order.versionOrPlan}</p></div><button type="button" className="rounded-full p-1 text-slate-500 hover:bg-white" onClick={() => setQuickLicenceOrderId('')} aria-label="Close code entry"><X className="h-4 w-4" /></button></div>
              {(!isSalesOrder || quickLicenceKind === 'sales-id') && <div className="mt-3 rounded-xl border border-[#cbdcd9] bg-white p-3"><p className="text-xs font-bold text-slate-700">Next available {quickLicenceKind === 'sales-id' ? 'Sales ID' : 'licence'} in stock</p>{nextAvailableLicence ? <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><span className="font-mono text-xs text-slate-600">{nextAvailableLicence.maskedCode}</span><button type="button" disabled={busy === `quick-licence-${order.orderId}`} onClick={(event) => void assignQuickLicence(event, order, nextAvailableLicence.licenceId)} className={primaryButton}><KeyRound className="h-4 w-4" />{busy === `quick-licence-${order.orderId}` ? 'Adding…' : 'Use next available'}</button></div> : <p className="mt-2 text-xs text-amber-700">No available {quickLicenceKind === 'sales-id' ? 'Sales ID' : 'licence'} is in stock for this version.</p>}</div>}
              {quickLicenceKind === 'licence' && <><div className="my-3 flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400"><span className="h-px flex-1 bg-slate-200" />paste a licence<span className="h-px flex-1 bg-slate-200" /></div><textarea className={`${inputClass} min-h-24 font-mono`} value={quickLicenceValue} onChange={(event) => setQuickLicenceValue(event.target.value)} placeholder="Paste a licence code" /></>}
              {quickLicenceMessage && <p role="alert" className="mt-2 text-xs font-bold text-rose-700">{quickLicenceMessage}</p>}
              {quickLicenceKind === 'licence' && <button type="button" disabled={busy === `quick-licence-${order.orderId}` || !quickLicenceValue.trim()} onClick={(event) => void assignQuickLicence(event, order)} className={`${primaryButton} mt-3 w-full sm:w-auto`}><Plus className="h-4 w-4" />{busy === `quick-licence-${order.orderId}` ? 'Adding…' : 'Add pasted licence'}</button>}
            </section>}
          </div>;
        })}
      </div>

      {offlineOrderId && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#002b2b]/60 p-4 backdrop-blur-sm"><section role="dialog" aria-modal="true" aria-labelledby="offline-preview-title" className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-start justify-between"><div><h3 id="offline-preview-title" className="text-lg font-black text-[#014040]">Record offline payment</h3><p className="mt-1 font-mono text-xs text-slate-500">{offlineOrderId}</p></div><button type="button" onClick={() => setOfflineOrderId('')} aria-label="Close offline payment" className="rounded-lg p-2 hover:bg-slate-100"><X className="h-5 w-5" /></button></div><p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs text-amber-900">Confirm the payment independently before recording it. This will mark the order paid.</p><div className="mt-4 space-y-3"><label className={labelClass}>MoMo transaction ID (optional)<input className={inputClass} value={offlineReference} onChange={(event) => setOfflineReference(event.target.value)} placeholder="Leave blank if unavailable" /></label><label className={labelClass}>Reason<input className={inputClass} value={offlineReason} onChange={(event) => setOfflineReason(event.target.value)} /></label></div>{offlineError && <p role="alert" className="mt-3 text-xs font-bold text-rose-700">{offlineError}</p>}<div className="mt-5 flex gap-2"><button type="button" onClick={() => setOfflineOrderId('')} className={`${secondaryButton} flex-1`}>Cancel</button><button type="button" onClick={() => void recordPreviewPayment()} disabled={busy === 'preview-offline' || !offlineReason.trim()} className={`${primaryButton} flex-1`}>{busy === 'preview-offline' ? 'Recording…' : 'Confirm payment'}</button></div></section></div>}
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[#002b2b]/60 p-3 sm:p-8">
          <section className="mx-auto max-w-4xl rounded-3xl bg-white p-5 shadow-2xl sm:p-8">
            ...
            <div className="mt-6 grid gap-4 rounded-2xl bg-slate-50 p-4 text-sm md:grid-cols-3">
              ...
              <div className="md:col-span-3"><b>Payment reference</b><p className="break-all font-mono text-xs">{selected.paystackReference || selected.offlinePaymentReference || '—'}</p>{selected.paymentMethod === 'offline' && selected.offlinePaymentReference && <button type="button" className={`${secondaryButton} mt-2`} onClick={() => startGuardedEdit('momo-reference')}>Edit MoMo transaction ID</button>}</div>
              ...
              {selected.salesCode && <div><b>Sales ID</b><p className="break-all font-mono text-xs">{selected.salesCode}</p><button type="button" className={`${secondaryButton} mt-2`} onClick={() => startGuardedEdit('sales-id')}>Edit or remove Sales ID</button></div>}
              ...
              {selected.activationCodeOrKey && <div className="md:col-span-3"><b>Attached licence / activation code</b><p className="break-all font-mono text-xs">{selected.activationCodeOrKey}</p><button type="button" className={`${secondaryButton} mt-2`} onClick={() => startGuardedEdit('licence')}>Edit or remove licence</button></div>}
            ...
            <section className="mt-5 rounded-2xl border border-[#cbdcd9] p-4 sm:p-5"><div className="flex flex-wrap gap-2">{orderSteps.map((item, index) => <button key={item.id} type="button" onClick={() => setStep(index)} className={`rounded-full px-3 py-2 text-xs font-black ${step === index ? 'bg-[#014040] text-white' : 'bg-slate-100 text-slate-600'}`}>{index + 1}. {item.label}</button>)}</div>
              <div className="mt-5">
                ...
                {activeStep === 'activation' && <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  {usesSalesId && <label className={labelClass}>Sales ID (admin only)<input className={inputClass} readOnly={Boolean(selected.salesCode)} value={workflow.salesCode || ''} placeholder={selectedVariant?.autoFulfil ? 'Assigned automatically after payment' : 'Enter a Sales ID or choose one below'} onChange={(e) => setWorkflow((old) => ({ ...old, salesCode: e.target.value }))} />{selected.salesCode && <span className="text-[10px] text-amber-700">Use guarded edit above to change an assigned Sales ID.</span>}</label>}
                  <label className={`${labelClass} ${usesSalesId ? 'sm:col-span-2' : ''}`}>Licence code<input className={inputClass} readOnly={Boolean(selected.activationCodeOrKey)} value={workflow.activationCodeOrKey || ''} placeholder={selected.activationCodeOrKey ? 'A licence code is already saved; use guarded edit above' : 'Enter the generated licence code'} onChange={(e) => setWorkflow((old) => ({ ...old, activationCodeOrKey: e.target.value }))} /></label>
      ...
      {selected && editKind && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#002b2b]/70 p-4 backdrop-blur-sm"><section role="dialog" aria-modal="true" aria-labelledby="guarded-edit-title" className="w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><h3 id="guarded-edit-title" className="text-lg font-black text-[#014040]">Correct {editKind === 'momo-reference' ? 'MoMo transaction ID' : editKind === 'sales-id' ? 'Sales ID' : 'licence'}</h3><p className="mt-1 font-mono text-xs text-slate-500">{selected.orderId}</p></div><button type="button" onClick={() => { setEditKind(null); setEditPassword(''); }} aria-label="Close correction" className="rounded-lg p-2 hover:bg-slate-100"><X className="h-5 w-5" /></button></div><p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs leading-5 text-amber-900"><AlertTriangle className="mr-1 inline h-4 w-4" />This value may already have been used or delivered. Check the customer and your records before replacing or removing it. The change is audited.</p><div className="mt-4 space-y-3"><label className={labelClass}>{editKind === 'momo-reference' ? 'Correct transaction ID' : 'Replacement code (leave blank to remove)'}<input className={inputClass} value={editValue} onChange={(event) => setEditValue(event.target.value)} autoComplete="off" /></label><label className={labelClass}>Reason for correction<input className={inputClass} value={editReason} onChange={(event) => setEditReason(event.target.value)} /></label><label className={labelClass}>Admin email<input className={inputClass} type="email" value={editEmail} onChange={(event) => setEditEmail(event.target.value)} autoComplete="username" /></label><label className={labelClass}>Admin password<input className={inputClass} type="password" value={editPassword} onChange={(event) => setEditPassword(event.target.value)} autoComplete="current-password" /></label></div>{editError && <p role="alert" className="mt-3 text-xs font-bold text-rose-700">{editError}</p>}<div className="mt-5 flex gap-2"><button type="button" onClick={() => { setEditKind(null); setEditPassword(''); }} className={`${secondaryButton} flex-1`}>Cancel</button><button type="button" onClick={() => void saveGuardedEdit()} disabled={busy === 'guarded-edit' || !editReason.trim() || !editEmail.trim() || !editPassword} className={`${primaryButton} flex-1`}>{busy === 'guarded-edit' ? 'Checking…' : editValue.trim() ? 'Save correction' : 'Remove value'}</button></div></section></div>}
    </div>
  );
}
...
function WholeNumberInput({ value, onChange, min = 0, max = 1_000_000 }: { value: number; onChange: (value: number) => void; min?: number; max?: number }) {
  const [draft, setDraft] = useState(String(value));
  useEffect(() => setDraft(String(value)), [value]);
  const commit = (raw: string) => {
    const parsed = Number(raw);
    const next = Number.isFinite(parsed) ? Math.max(min, Math.min(max, Math.trunc(parsed))) : 0;
    setDraft(String(next));
    onChange(next);
  };
  return <input className={inputClass} type="text" inputMode={min < 0 ? 'decimal' : 'numeric'} pattern={min < 0 ? '-?[0-9]*' : '[0-9]*'} value={draft} onChange={(event) => {
    const raw = event.target.value;
    if (!(min < 0 ? /^-?\d*$/.test(raw) : /^\d*$/.test(raw))) return;
    setDraft(raw);
    if (raw !== '' && raw !== '-') onChange(Math.max(min, Math.min(max, Number(raw))));
  }} onBlur={() => commit(draft)} />;
}
...
function PricingPromotionsSection({ data, user, reload }: { data: AdminData; user: User; reload: () => Promise<void> }) {
  ...
  const groupedItems = data.categories.map((category) => ({
    category,
    items: catalogueItems.filter((item) => item.categoryId === category.categoryId)
  ...
  return <div className="space-y-6">
    ...
    <section className="space-y-4 rounded-2xl border bg-white p-5">
      ...
      <div className="grid gap-3 md:grid-cols-3"><label className={labelClass}>Discount (%)<WholeNumberInput value={pricing.globalPromotion.percent} max={100} onChange={(percent) => setPricing((old) => ({ ...old, globalPromotion: { ...old.globalPromotion, percent } }))} /></label><label className={labelClass}>Promotion description<input className={inputClass} value={pricing.globalPromotion.label} placeholder="Back to school promo" onChange={(event) => setPricing((old) => ({ ...old, globalPromotion: { ...old.globalPromotion, label: event.target.value } }))} /></label><label className={labelClass}>End date and time<input className={inputClass} type="datetime-local" value={dateTimeLocal(pricing.globalPromotion.endsAt)} onChange={(event) => setPricing((old) => ({ ...old, globalPromotion: { ...old.globalPromotion, endsAt: event.target.value ? new Date(event.target.value).toISOString() : undefined } }))} /></label></div>
      <details className="rounded-xl bg-slate-50 p-3"><summary className="cursor-pointer text-sm font-black text-[#014040]">Choose products and services ({pricing.globalPromotion.targetIds.length || 'all'})</summary>
        <div className="mt-3 space-y-2"><label className="flex items-center gap-2 rounded-lg bg-white p-2 text-xs font-bold"><input type="checkbox" checked={pricing.globalPromotion.targetIds.length === 0} onChange={() => setPricing((old) => ({ ...old, globalPromotion: { ...old.globalPromotion, targetIds: [] } }))} />All catalogue items</label>
          {groupedItems.map(({ category, items }) => <details key={category.categoryId} className="rounded-lg border border-slate-200 bg-white p-3"><summary className="cursor-pointer text-sm font-bold text-[#014040]">{category.name} · {items.length}</summary><div className="mt-3 grid gap-2 sm:grid-cols-2">{items.map((item) => { const targetId = pricingTargetId(item); return <label key={`${item.kind}:${item.itemId}`} className="flex items-center gap-2 rounded-lg bg-slate-50 p-2 text-xs"><input type="checkbox" checked={pricing.globalPromotion.targetIds.includes(targetId)} onChange={() => toggleGlobalTarget(targetId)} /><span>{item.name}</span></label>; })}</div></details>)}
        </div>
      </details>
    </section>
    ...
    <section className="space-y-4 rounded-2xl border bg-white p-5">
      ...
      <div className="space-y-2"><p className="text-xs font-bold text-slate-700">Choose a product or service</p>{groupedItems.map(({ category, items }) => <details key={category.categoryId} className="rounded-lg border border-slate-200 bg-slate-50 p-3"><summary className="cursor-pointer text-sm font-bold text-[#014040]">{category.name} · {items.length}</summary><div className="mt-3 grid gap-2 sm:grid-cols-2">{items.map((item) => { const targetId = pricingTargetId(item); return <label key={`${item.kind}:${item.itemId}`} className={`flex cursor-pointer items-center gap-2 rounded-lg p-2 text-xs ${selectedItemId === targetId ? 'bg-[#e0f5ee] font-black text-[#014040]' : 'bg-white'}`}><input type="radio" name="item-promotion-target" checked={selectedItemId === targetId} onChange={() => setSelectedItemId(targetId)} />{item.name}</label>; })}</div></details>)}</div>
      <p className="text-xs font-bold text-[#014040]">Editing: {catalogueItems.find((item) => pricingTargetId(item) === selectedItemId)?.name || 'Select a product'}</p>
      <div className="grid gap-3 md:grid-cols-[110px_1fr_1fr_1fr]"><label className={labelClass}>This promo<span className="flex h-[38px] items-center gap-2 rounded-xl border px-3"><input type="checkbox" checked={selectedRule.active} onChange={(event) => setSelectedRule({ active: event.target.checked })} />{selectedRule.active ? 'On' : 'Off'}</span></label><label className={labelClass}>Discount (%)<WholeNumberInput key={selectedItemId} value={selectedRule.percent} max={100} onChange={(percent) => setSelectedRule({ percent })} /></label><label className={labelClass}>Description<input className={inputClass} value={selectedRule.label} placeholder="Special offer" onChange={(event) => setSelectedRule({ label: event.target.value })} /></label><label className={labelClass}>End date and time<input className={inputClass} type="datetime-local" value={dateTimeLocal(selectedRule.endsAt)} onChange={(event) => setSelectedRule({ endsAt: event.target.value ? new Date(event.target.value).toISOString() : undefined })} /></label></div>
    </section>
    ...
    <section className="space-y-4 rounded-2xl border bg-white p-5">
      ...
      <div className="grid gap-3 md:grid-cols-[180px_1fr]"><label className={labelClass}>Global adjustment (%)<WholeNumberInput value={pricing.silentAdjustment.percent} min={-99} max={1000} onChange={(percent) => setPricing((old) => ({ ...old, silentAdjustment: { ...old.silentAdjustment, percent } }))} /><span className="font-normal text-slate-500">Type a negative number to decrease prices.</span></label><div><p className="mb-1 text-xs font-bold text-slate-700">Categories</p><div className="flex flex-wrap gap-2"><label className="rounded-lg border px-3 py-2 text-xs font-bold"><input className="mr-2" type="checkbox" checked={pricing.silentAdjustment.targetIds.length === 0} onChange={() => setPricing((old) => ({ ...old, silentAdjustment: { ...old.silentAdjustment, targetIds: [] } }))} />All</label>{data.categories.map((category) => <label key={category.categoryId} className="rounded-lg border px-3 py-2 text-xs font-bold"><input className="mr-2" type="checkbox" checked={pricing.silentAdjustment.targetIds.includes(`CATEGORY:${category.categoryId}`)} onChange={() => toggleSilentCategory(category.categoryId)} />{category.name}</label>)}</div></div></div>
      <details className="rounded-xl bg-slate-50 p-3"><summary className="cursor-pointer text-sm font-black text-[#014040]">Fixed adjustments by product, service, or software version</summary><p className="mt-2 text-xs text-slate-500">Enter a whole-cedi amount: positive to increase, negative to decrease, or 0 to disable. Product and version amounts combine when both are set.</p><div className="mt-3 space-y-2">{groupedItems.map(({ category, items }) => <details key={category.categoryId} className="rounded-lg border border-slate-200 bg-white p-3"><summary className="cursor-pointer text-sm font-black text-[#014040]">{category.name} · {items.length}</summary><div className="mt-3 space-y-2">{items.map((item) => { const targetId = pricingTargetId(item); const variants = item.kind === 'product' ? data.products.find((product) => product.productId === item.itemId)?.variants || [] : []; const adjustment = (id: string, label: string) => <label key={id} className="grid items-center gap-2 rounded-lg bg-slate-50 p-2 text-xs font-bold sm:grid-cols-[1fr_150px]"><span>{label}</span><span className="flex items-center gap-2">GHS <WholeNumberInput value={pricing.silentAdjustment.fixedAdjustmentsGhs?.[id] || 0} min={-1_000_000} onChange={(amount) => setFixedAdjustment(id, amount)} /></span></label>; return <details key={`${item.kind}:${item.itemId}`} className="rounded-lg border border-slate-100 p-2"><summary className="cursor-pointer text-xs font-bold">{item.name}{variants.length ? ` · ${variants.length} versions` : ''}</summary><div className="mt-2 space-y-2">{adjustment(targetId, 'All versions / product')}{variants.map((variant) => { const current = applyPricingRules(Math.round(variant.priceGhs * 100), [item.itemId, variant.variantId, `CATEGORY:${category.categoryId}`], data.pricing).payablePesewas; return adjustment(variant.variantId, `${variant.versionOrPlan} · ${variant.os || 'OS not specified'} · Current ${formatPesewas(current)}`); })}</div></details>; })}</div></details>)}</div></details>
    </section>
    {message && <p role="status" className="rounded-xl bg-slate-100 p-3 text-sm font-bold">{message}</p>}
  </div>;
}
